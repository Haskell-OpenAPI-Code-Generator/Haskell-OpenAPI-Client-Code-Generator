  ...
  hspec $ do
    describe "runPostPaymentIntent"
      ...
      $ do
        ...
        (getResponseBody . fromRight undefined) response `shouldBe` PostPaymentIntentsResponse200 paymentIntent
    ...
    describe "runCheckoutSession" $ do
      it "should encode Body" $ do
        ...
        (getResponseBody . fromRight undefined) response `shouldBe` PostCheckoutSessionsResponseError "Error in $: key \"cancel_url\" not found"
      it "should run Raw Response" $ do
        ...
        (getResponseBody . fromRight undefined) response `shouldBe` succeededResponseBody
